...
- **FR-010**: The Kong configuration MUST be expressed as a `deck` declarative config file (`kong/api-gateway/deck/kong.yaml`) and applied to the Konnect control plane via `deck sync --konnect-control-plane-name plaudelm`. `deck validate` MUST be run for syntax checking before every sync. `deck diff --konnect-control-plane-name plaudelm` MUST be run after sync to verify zero drift. There is no local admin API — this is a Konnect-managed data plane.
